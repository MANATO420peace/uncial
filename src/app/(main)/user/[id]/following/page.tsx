...
export const metadata: Metadata = { title: 'フォロー中' }
...
export default async function FollowingPage({ params }: Props) {
  ...
  const following = await getFollowList(id, 'following')
  ...
  return (
    <div>
      ...
      {following.length === 0 ? (
        <p className="text-center py-10 text-sm text-muted-foreground">フォロー中のユーザーはいません</p>
      ) : (
        <ul className="divide-y">
          {following.map((user: never) => {
